...
export function buildFairPackages({
  ...
  const payerBp = payerTop ? DRAFT_BLUEPRINTS[payerTop.id] : null;
  const receiverBp = receiverTop ? DRAFT_BLUEPRINTS[receiverTop.id] : null;
  const receiverPhase = packageReceiver.teamPhase?.phase || null;
  const receiverRoles = computeLineupRoles(packageReceiver.enriched, leagueContext);
  ...
  const lo = anchorValue * 0.72;
  const hi = anchorValue * 1.28;
  const combos = [];
